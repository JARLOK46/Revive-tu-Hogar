...
<head>
  ...
  <title>Proyectos - Revive tu Hogar</title>
...
<body>
  ...
  <header class="navbar">
    <div class="container navbar-inner">
      ...
      <nav class="nav-links">
        ...
        <a href="/proyectos.php" class="badge">Proyectos</a>
  ...
  <main class="container section">
    <section class="hero-page">
      <h1 class="reveal-left">Proyectos realizados</h1>
      <p class="lead reveal-right" style="transition-delay:60ms">Algunas de nuestras intervenciones recientes en cocinas, salas y baños.</p>
    </section>

    <section class="gallery stagger reveal" style="margin-top:10px">
      <?php
        // Cargar exclusivamente desde la tabla de proyectos
        $items = [];
        try {
          if (isset($pdo) && $pdo instanceof PDO) {
            $items = $pdo->query('SELECT g.titulo, g.descripcion, g.imagen_url, c.nombre AS cliente_nombre FROM proyectos_galeria g LEFT JOIN clientes c ON c.id=g.cliente_id ORDER BY g.created_at DESC, g.id DESC')->fetchAll(PDO::FETCH_ASSOC) ?: [];
          }
        } catch (Throwable $e) {}
        ...
        if (!empty($items)):
          foreach ($items as $it):
      ?>
        <article class="project-card reveal" data-3d>
      ...
      <?php endforeach; else: ?>
        <div class="empty-state reveal">
          ...
          <p class="subtle">Pronto compartiremos nuevas intervenciones.</p>
        </div>
      <?php endif; ?>
    </section>

    <div class="cta-block glass reveal" style="margin-top:22px">
      <p>¿Quieres ver tu proyecto aquí? Conversemos tu idea y te damos una propuesta.</p>
      <a class="btn btn-secondary shine" href="/proceso.php">Conoce el proceso</a>
    </div>
  </main>
