...
<body>
  ...
  <?php
  // Cargar planes desde BD para el home (máximo 3)
  $planesHome = [];
  try {
    if ($pdo instanceof PDO) {
      $stm = $pdo->query('SELECT id, nombre_plan, descripcion, precio, duracion_dias FROM planes ORDER BY precio ASC LIMIT 3');
      $planesHome = $stm->fetchAll(PDO::FETCH_ASSOC) ?: [];
    }
  } catch (Throwable $e) {
    $planesHome = [];
  }
  // Helper para CTA de contratar desde home
  function contratar_home_link($planId): string {
    if (isset($_SESSION['user_id'])) {
      if (!empty($_SESSION['rol']) && $_SESSION['rol'] !== 'cliente') {
        // redirige al dashboard correspondiente si no es cliente
        $d = '/cliente/dashboard.php';
        if (isset($_SESSION['rol']) && $_SESSION['rol'] === 'admin') $d = '/admin/index.php';
        elseif (isset($_SESSION['rol']) && $_SESSION['rol'] === 'empleado') $d = '/empleado/dashboard.php';
        return $d;
      }
  ...
  <main id="main" tabindex="-1">
    <section class="hero container theme-bg">
      <div>
        <h1 class="reveal-left">Transformamos espacios, mejoramos vidas</h1>
        <p class="lead reveal-right" style="transition-delay:60ms">Diseño, mantenimiento y renovación con un enfoque cálido, moderno y minimalista.</p>
        <ul class="features-list reveal-left" style="margin-top:10px;transition-delay:120ms">
          <li>Diseño y ejecución con garantía</li>
          <li>Cronograma claro y comunicación continua</li>
          <li>Materiales de calidad y equipos certificados</li>
        </ul>
        <div class="reveal-right" style="margin-top:12px;display:flex;gap:10px;flex-wrap:wrap;transition-delay:180ms">
          <a class="btn grad shine" href="/planes.php">Ver planes</a>
          <a class="btn btn-secondary shine" href="/proyectos.php">Ver proyectos</a>
        </div>
      </div>
      <div class="card theme-surface float-y reveal" data-3d style="transition-delay:120ms">
        <strong class="badge">Planes destacados</strong>
        <ul>
          <?php if (!empty($planesHome)): ?>
            <?php foreach ($planesHome as $pl): ?>
              <li><?php echo htmlspecialchars($pl['nombre_plan'] ?? '', ENT_QUOTES, 'UTF-8'); ?>: <?php echo htmlspecialchars($pl['descripcion'] ?? '', ENT_QUOTES, 'UTF-8'); ?></li>
            <?php endforeach; ?>
          <?php else: ?>
            <li>Consulta nuestros planes disponibles.</li>
          <?php endif; ?>
        </ul>
        <a class="btn grad shine" href="/planes.php" style="margin-top:10px;display:inline-block">Ver detalles y comparación</a>
      </div>
    </section>

    <section class="stats section theme-bg" aria-label="Indicadores de confianza">
      <div class="container">
        <div class="stats-grid">
          <div class="stat reveal"><strong data-target="120" data-suffix="+">0</strong><span>Proyectos entregados</span></div>
          <div class="stat reveal" style="transition-delay:60ms"><strong data-target="4.8" data-decimals="1" data-suffix="/5">0</strong><span>Satisfacción promedio</span></div>
          <div class="stat reveal" style="transition-delay:120ms"><strong data-target="7" data-suffix=" años">0</strong><span>Experiencia del equipo</span></div>
          <div class="stat reveal" style="transition-delay:180ms"><strong data-target="48" data-suffix=" h">0</strong><span>Tiempo de respuesta</span></div>
        </div>
      </div>
    </section>

    <section id="sobre" class="section theme-bg">
      <div class="container">
        <h2 class="reveal-left">Sobre nosotros</h2>
        <p class="reveal-right" style="max-width:820px">Somos un equipo apasionado por revitalizar hogares. Combinamos funcionalidad y estética para que disfrutes cada rincón.</p>
        <div class="grid cards stagger reveal" style="margin-top:12px">
          <div class="card theme-surface reveal-scale" data-3d>
            <h3>Nuestra misión</h3>
            <p>Hacer que renovar tu casa sea simple, claro y alcanzable. Te guiamos con criterios profesionales para tomar mejores decisiones de color, distribución y compra.</p>
          </div>
          <div class="card theme-surface reveal-scale" data-3d>
            <h3>Metodología</h3>
            <ul class="features-list">
              <li>Diagnóstico por videollamada</li>
              <li>Propuesta de color y estilo</li>
              <li>Recomendaciones de distribución</li>
              <li>Lista de productos opcional</li>
            </ul>
          </div>
          <div class="card theme-surface reveal-scale" data-3d>
            <h3>Lo que nos diferencia</h3>
            <ul class="features-list">
              <li>Lenguaje claro y acompañamiento real</li>
              <li>Enfoque en asesoría (no vendemos materiales)</li>
              <li>Recomendaciones ajustadas al presupuesto</li>
            </ul>
          </div>
        </div>
        <div class="grid cards stagger reveal" style="margin-top:12px">
          <div class="card theme-surface reveal-scale" data-3d>
            ...
            <p class="subtle">Transparencia, cercanía y compromiso con resultados de calidad.</p>
          </div>
          <div class="card theme-surface reveal-scale" data-3d>
            <div class="kicker">Equipo</div>
            <p class="subtle">Diseñadores y técnicos con más de 7 años de experiencia combinada.</p>
          </div>
          <div class="card theme-surface reveal-scale" data-3d>
            <div class="kicker">Garantía</div>
            <p class="subtle">Si algo no te convence de la propuesta, hacemos un ajuste sin costo.</p>
          </div>
        </div>
        <p class="reveal" style="margin-top:10px"><a class="btn grad shine" href="#planes">Conoce nuestros planes</a></p>
      </div>
    </section>

    <section id="servicios" class="section theme-bg">
      <div class="container">
        <h2 class="reveal">Servicios</h2>
        <div class="grid cards stagger reveal">
          <div class="card theme-surface reveal-scale" data-3d><h3>Mantenimiento</h3><ul class="features-list"><li>Visitas programadas</li><li>Chequeos preventivos</li><li>Reparaciones menores</li></ul></div>
          <div class="card theme-surface reveal-scale" data-3d><h3>Renovación</h3><ul class="features-list"><li>Actualización de espacios</li><li>Materiales certificados</li><li>Acabados durables</li></ul></div>
          <div class="card theme-surface reveal-scale" data-3d><h3>Asesoría</h3><ul class="features-list"><li>Concepto y estilo</li><li>Selección de materiales</li><li>Optimización de presupuesto</li></ul></div>
        </div>
        <p class="reveal" style="margin-top:10px"><a class="btn grad shine" href="/servicios.php">Ver todos los servicios</a></p>
      </div>
    </section>

    <section id="asesorias" class="section theme-bg">
      <div class="container">
        <h2 class="reveal">¿Cómo funcionan nuestras asesorías?</h2>
        <div class="grid cards stagger reveal" style="margin-top:10px">
          <div class="card theme-surface reveal-scale" data-3d>
            <h3>1) Diagnóstico por videollamada</h3>
            <p>Conversamos 30–60 min para entender tus necesidades, estilo y presupuesto.</p>
          </div>
          <div class="card theme-surface reveal-scale" data-3d>
            <h3>2) Propuesta de color y distribución</h3>
            <p>Recibes paleta de colores, referencias y recomendaciones de distribución.</p>
          </div>
          <div class="card theme-surface reveal-scale" data-3d>
            <h3>3) Lista de productos opcional</h3>
            <p>Te sugerimos mobiliario y accesorios con alternativas de compra.</p>
          </div>
          <div class="card theme-surface reveal-scale" data-3d>
            <h3>4) Seguimiento</h3>
            <p>Resolución de dudas y ajustes menores para que ejecutes con confianza.</p>
          </div>
        </div>
        <p class="subtle reveal" style="margin-top:6px">Nota: brindamos asesoría y recomendaciones. No suministramos materiales de obra.</p>
      </div>
    </section>

    <section id="planes" class="section theme-bg">
      <div class="container">
        <h2 class="reveal">Planes</h2>
        <p class="reveal subtle" style="max-width:820px">Nuestros planes están enfocados en asesorías: te guiamos con paletas de color, distribución y selección de productos. No ofrecemos materiales de obra.</p>
        <div class="grid cards stagger reveal" id="planes-grid">
          <?php if (!empty($planesHome)): ?>
            <?php foreach ($planesHome as $pl): ?>
              <div class="card theme-surface reveal-scale" data-3d>
                <h3><?php echo htmlspecialchars($pl['nombre_plan'] ?? '', ENT_QUOTES, 'UTF-8'); ?></h3>
                <p>Desde <?php echo '$' . number_format((float)($pl['precio'] ?? 0), 0); ?>/<?php echo ((int)($pl['duracion_dias'] ?? 0) === 30) ? 'mes' : ((int)($pl['duracion_dias'] ?? 0) . ' días'); ?></p>
                <a class="btn grad shine" href="<?php echo htmlspecialchars(contratar_home_link((int)$pl['id']), ENT_QUOTES, 'UTF-8'); ?>">Contratar</a>
              </div>
            <?php endforeach; ?>
          <?php else: ?>
            <div class="card theme-surface reveal-scale" data-3d><h3>Consulta nuestros planes</h3><p>Pronto mostraremos nuestros planes disponibles.</p><a class="btn grad shine" href="/planes.php">Ver planes</a></div>
          <?php endif; ?>
        </div>
        <p class="reveal" style="margin-top:10px"><a class="btn grad shine" href="/planes.php">Comparativa detallada de planes</a></p>
      </div>
    </section>

    <section id="proyectos" class="section theme-bg">
      <div class="container">
        <h2 class="reveal">Proyectos</h2>
        <section class="gallery stagger reveal" style="margin-top:10px">
          <?php
            // Primero intentar cargar desde DB si existe la tabla
            $items = [];
            try {
              if (isset($pdo) && $pdo instanceof PDO) {
                $items = $pdo->query('SELECT g.titulo, g.descripcion, g.imagen_url, c.nombre AS cliente_nombre FROM proyectos_galeria g LEFT JOIN clientes c ON c.id=g.cliente_id ORDER BY g.created_at DESC, g.id DESC LIMIT 6')->fetchAll(PDO::FETCH_ASSOC) ?: [];
              }
            } catch (Throwable $e) {}
          ...
          ?>
            <article class="project-card reveal">
              <div class="project-thumb" style="background:#eee; display:flex; align-items:center; justify-content:center; overflow:hidden;">
                <img src="<?php echo htmlspecialchars($it['imagen_url']); ?>" alt="Proyecto: <?php echo htmlspecialchars($it['titulo']); ?>" style="width:100%; height:auto; object-fit:cover;" />
              </div>
              <strong><?php echo htmlspecialchars($it['titulo']); ?></strong>
              <div class="project-meta">
                <?php if (!empty($it['cliente_nombre'])): ?><span class="tag">@<?php echo htmlspecialchars($it['cliente_nombre']); ?></span><?php endif; ?>
              </div>
              <?php if (!empty($it['descripcion'])): ?><p class="subtle"><?php echo htmlspecialchars($it['descripcion']); ?></p><?php endif; ?>
            </article>
          <?php endforeach; else: ?>
            <div class="empty-state reveal">
              <div class="empty-icon">📭</div>
              <h3>Sin proyectos publicados</h3>
              <p class="subtle">Cuando haya proyectos disponibles los verás aquí.</p>
            </div>
          <?php endif; ?>
        </section>
        <p class="reveal" style="margin-top:10px"><a class="btn btn-secondary shine" href="/proyectos.php">Ver más proyectos</a></p>
      </div>
    </section>



    <div class="cta-block glass theme-surface reveal" style="margin: 0 20px 10px">
      <p>¿Listo para comenzar? Revisa los planes o escríbenos tu idea y te contactamos.</p>
      <a class="btn grad shine" href="/planes.php">Empezar</a>
    </div>
  </main>
  ...
  <script type="module">
    ...
    const ChatSessions = {
      loadSessions() {
        try { return JSON.parse(localStorage.getItem(SESS_LIST_KEY)) || []; } catch { return []; }
      },
      saveSessions(list) {
        try { localStorage.setItem(SESS_LIST_KEY, JSON.stringify(list)); } catch {}
      },
      ensureActiveSession() {
        let id = localStorage.getItem(SESS_CURR_KEY);
        const sessions = this.loadSessions();
        if (!id || !sessions.find(s => s.id === id)) {
          // Migración inicial desde mensajes globales (si existieran)
      ...
      getMessages(id) {
        try { return JSON.parse(localStorage.getItem(SESS_MSG_PREFIX + id)) || []; } catch { return []; }
      },
      saveMessages(id, messages) {
        try {
          localStorage.setItem(SESS_MSG_PREFIX + id, JSON.stringify(messages));
          const sessions = this.loadSessions();
          const s = sessions.find(x => x.id === id);
          if (s) { s.updatedAt = Date.now(); if (!s.title) s.title = 'Chat ' + (sessions.indexOf(s) + 1); this.saveSessions(sessions); }
        } catch {}
      },
      addMessage(id, message) {
        const msgs = this.getMessages(id);
        const m = { text: String(message.text || ''), sender: String(message.sender || 'user'), timestamp: message.timestamp || Date.now() };
      ...
      createNewSession(title) {
        ...
        const t = title || 'Chat ' + (sessions.length + 1);
        sessions.push({ id, title: t, createdAt: Date.now(), updatedAt: Date.now() });
        this.saveSessions(sessions);
    ...
    // Monkey-patch para que los mensajes se guarden también en la sesión activa
    (function(){
      if (!window.currentChatSessionId) ChatSessions.ensureActiveSession();
      try {
        const _origAdd = ChatStorage.addMessage.bind(ChatStorage);
        ChatStorage.addMessage = function(message) {
          try { _origAdd(message); } catch(e) {}
          try { const sid = window.currentChatSessionId || ChatSessions.ensureActiveSession(); ChatSessions.addMessage(sid, message); } catch(e) { console.warn('ChatSessions.addMessage error', e); }
        };
      } catch(e) { console.warn('Monkey-patch ChatStorage.addMessage falló', e); }
    })();

    // Helpers UI para selector de sesiones y botón "Nuevo chat"
    function initChatSessionUI(){ try{ const controlsId='chat-session-controls'; if(document.getElementById(controlsId)) return; const c=document.createElement('div'); c.id=controlsId; c.className='chat-session-controls'; c.innerHTML=`<select id="chat-session-select" class="chat-session-select"></select><button id="chat-session-new" class="chat-session-new">Nuevo chat</button>`;
        ...
        (function(){
          function attachInline(header){
            c.classList.add('chat-session-controls--inline');
            c.classList.remove('chat-session-controls--overlay');
          ...
          function tryInsert(){
            ...
            for (const root of roots){
              ...
              if (header){ attachInline(header); return true; }
            }
            return false;
          }
          if (!tryInsert()){
            ...
            const iv=setInterval(()=>{
              ...
              if (tryInsert() || attempts>=20){
                ...
                if (!c.parentNode){
                  ...
                  // Asegurar posicionamiento relativo para ubicar overlay dentro del contenedor del chat
                  ...
                  host.appendChild(c);
                  // Observar aparición tardía del header para reubicar controles dentro de él
                  try{
                    const rootsToWatch=[document, host];
                    if (host && host.shadowRoot) rootsToWatch.push(host.shadowRoot);
                    const mo = new MutationObserver(()=>{ if (tryInsert()){ try{ mo.disconnect(); }catch(_){ } } });
                    rootsToWatch.forEach(r=>{ if(r && r.nodeType===1){ mo.observe(r, {childList:true, subtree:true}); } });
                  }catch(_){ }
                }
              }
            },250);
    ...
    chatInstance = window.chatInstance = createChat({
      webhookUrl: 'http://localhost:5678/webhook/43b6b7f6-6a87-44c7-a36a-d17704226742/chat',
      webhookConfig: {
        method: 'POST',
        headers: {
